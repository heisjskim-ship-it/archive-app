...
export default function App() {
  ...
  const [studentNewQuestion, setStudentNewQuestion] = useState<{
    ...
    imagePreviews: string[];
    isShared: boolean;
  }>({ title: '', images: [], imagePreviews: [], isShared: true });
  ...
  // 💡 리액티브 파생 바인딩 (실시간 스냅샷 submissions와 viewingSubmission 연동)
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative select-none">
            ...
            <div className="text-center mb-5">
              ...
              <div className="mt-2.5 px-3 py-2 bg-amber-50 text-amber-800 text-[11px] rounded-xl font-bold border border-amber-100 leading-relaxed text-left">
                {installSupportInfo.isIframe ? (
                  <p>⚠️ 현재 **실험실 플레이어 프레임 내부**에서 작동 중이어서 원클릭 자동 설치가 차단되었습니다. 실제 상단 주소창을 통해 새 탭으로 열거나 정식 도메인으로 접속 시 PC/안드로이드에서 원클릭 바로가기 설치가 작동합니다.</p>
                ) : installSupportInfo.isIOS ? (
                  <p>🍎 아이폰(iOS) 사파리 브라우저는 애플 보안 정책상 자동 설치 창을 지원하지 않습니다. 아래 수동 가이드를 통해 생성해 주세요.</p>
                ) : (
                  <p>💡 브라우저가 아직 앱 설치 준비를 감지하는 중이거나 현재 브라우저의 제한 사항으로 수동 가이드가 표시됩니다. 아래 가이드를 활용해 보세요!</p>
                )}
              </div>
            </div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="flex flex-col h-full space-y-4">
                          ...
                          {/* 💡 개편된 대화형 피드백/의견 교류창 (공간 극대화 및 미려한 디자인 기획) */}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-[70] overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl my-8">
            ...
            <form onSubmit={handleUpdateQuestionSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">이미지 목록</label>
                <div 
      ...
      {lightbox.show && (
        <div onClick={() => setLightbox({ show: false, imageUrl: '', title: '' })} className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex flex-col items-center justify-center p-4 z-[100] cursor-zoom-out">
          ...
          <img src={lightbox.imageUrl} alt="확대" className="max-h-full max-w-full object-contain rounded-xl shadow-2xl border-4 border-slate-800/50" />
          <div className="mt-4 text-center"><p className="text-white font-extrabold text-base">{lightbox.title}</p></div>
        </div>
      )}

      {}
      {/* 💡 전역 확인(Confirm) 모달 마크업 추가 (기출문제 삭제, 학생 제명, 비밀번호 초기화 작동 불가 버그 해결) */}
      {confirmModal.show && (
