...
// ==============================================
// 🎨 순수 SVG 아이콘 컴포넌트 내장 (lucide-react 대체)
// ==============================================
...
const uploadToCloudinary = async (file) => {
  ...
  // 세팅 전이거나 Canvas 프리뷰 환경일 때의 임시 처리 (로컬 미리보기)
  if (!cloudName || !uploadPreset) {
    console.warn("Cloudinary 키가 입력되지 않아 임시 URL을 사용합니다.");
  ...
  return data.secure_url; // 업로드된 이미지의 완벽한 HTTPS 링크 반환
};
...
const TUTORIAL_STEPS = {
  teacher: [
    ...
    { title: '1. 다중 문제 세트 등록', desc: '해시태그를 입력하고, 이미지를 드래그하거나 Ctrl+V로 등록하세요. 이미지는 Cloudinary 무료 서버에 안전하게 보관됩니다.', icon: <Upload className="text-emerald-500 mx-auto mb-4" size={56}/> },
    ...
    { title: '3. 회원 직접 관리 (엑셀형)', desc: '회원 관리 탭에서 엑셀처럼 표에 직접 입력하여 여러 명의 학생을 DB에 한 번에 가입시키고 관리할 수 있습니다.', icon: <Users className="text-blue-500 mx-auto mb-4" size={56}/> }
  ],
  student: [
    { title: '반가워요!', desc: '데이터베이스가 연동된 문제 풀이 아카이브입니다!', icon: <Sparkles className="text-amber-500 mx-auto mb-4" size={56}/> },
    { title: '1. 해시태그 검색 & 풀이 제출', desc: '해시태그로 과목이나 단원을 찾고, 종이에 푼 나만의 풀이를 스마트폰으로 사진 찍어 간편하게 제출해보세요.', icon: <Search className="text-indigo-500 mx-auto mb-4" size={56}/> },
    { title: '2. 선생님의 꼼꼼한 첨삭', desc: '제출한 풀이에 대해 선생님이 빨간펜 첨삭과 피드백 코멘트를 남겨주시면 내 오답노트에서 언제든 다시 볼 수 있습니다.', icon: <MessageSquare className="text-emerald-500 mx-auto mb-4" size={56}/> },
    { title: '3. 공개 챌린지! 친구들과 함께', desc: '챌린지 문제에 참여하면, 내 풀이를 공유하고 다른 친구들과 서로 피드백 댓글을 주고받으며 생각을 넓힐 수 있습니다!', icon: <Trophy className="text-amber-500 mx-auto mb-4" size={56}/> }
  ]
};
...
export default function App() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      if (authModal.mode === 'teacher_login' && loginIdInput.trim() === 'teacher' && loginPwInput.trim() === '1234') {
        setCurrentUser({ id: 'teacher_demo', name: '김수학', role: 'teacher' });
        setAuthModal({ show: false, mode: 'student_login' });
        alertMessage('선생님, 반갑습니다! 데모 교사 모드로 진입합니다.');
        return;
      }
  ...
  const completeTutorial = async () => {
    if (tutorial.role === 'student' && currentUser && currentUser.id !== 'teacher_demo') {
      await updateDoc(doc(getColRef('users'), currentUser.id), { hasSeenTutorial: true });
      alertMessage(`튜토리얼을 마쳤습니다. 아카이브에서 자유롭게 학습하세요!`);
    } else {
      setHasTeacherSeenTutorial(true);
      alertMessage('튜토리얼을 마쳤습니다. 멋진 문제들을 아카이브에 채워주세요!');
    }
  ...
  const handleSaveDraftStudents = async () => {
    ...
    for (const row of validRows) {
      ...
      try {
        const userCred = await createUserWithEmailAndPassword(auth, generateEmail(row.username.trim()), '1234');
    ...
    setIsLoading(false);
    alertMessage(`총 ${successCount}명의 학생 등록 완료 (비밀번호 1234)`);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative">
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex flex-col items-center justify-center z-[200]">
          ...
          <p className="text-white font-bold text-sm tracking-widest animate-pulse">데이터를 안전하게 처리 중...</p>
        </div>
      )}
          ...
          <div className="flex items-center gap-3">
            {!currentUser ? (
              <div className="flex items-center gap-2">
                <button onClick={() => setAuthModal({ show: true, mode: 'student_login' })} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1 hover:bg-indigo-700">
                  <LogIn size={13} /> 학생 로그인/가입
                </button>
                <button onClick={() => setAuthModal({ show: true, mode: 'teacher_login' })} className="px-4 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold shadow-sm flex items-center gap-1 hover:bg-emerald-700 hidden sm:flex">
                  <User size={13} /> 교사 로그인
                </button>
              </div>
            ) : (
                ...
                <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-fit">
                  ...
                  <form onSubmit={handleAddQuestion} className="space-y-5">
                    <div>
                      ...
                      <input type="text" value={newQuestion.title} onChange={e => setNewQuestion({...newQuestion, title: e.target.value})} className="w-full px-3 py-2.5 rounded-lg border focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="예: 2024 수능 22번" />
                    </div>
                    <div>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="relative max-w-[180px] w-full">
                        ...
                        <input type="text" value={teacherQuestionSearch} onChange={e => setTeacherQuestionSearch(e.target.value)} placeholder="제목, #태그 검색" className="w-full pl-8 pr-3 py-1.5 text-xs rounded-xl border bg-slate-50 outline-none focus:ring-1 focus:ring-emerald-500" />
                      </div>
                    </div>
                      ...
                      {filteredTeacherQuestions.length === 0 ? (
                        ...
                        filteredTeacherQuestions.map(q => (
                          <div key={q.id} className="py-2.5 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <img src={q.imageUrls[0]} alt="썸네일" className="w-10 h-10 object-cover rounded border cursor-zoom-in" onClick={()=>openLightbox(q.imageUrls[0], q.title)}/>
                              <div>
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <input type="text" value={submissionSearch} onChange={e=>setSubmissionSearch(e.target.value)} placeholder="학생명, 상태 검색" className="px-3 py-1.5 text-xs rounded-xl border focus:ring-1 outline-none" />
                    </div>
                    <div className="overflow-x-auto border rounded-xl">
                      <table className="w-full text-left text-xs">
                        <thead className="bg-slate-50 border-b text-slate-500">
                          <tr><th className="p-3">학생</th><th className="p-3">문제</th><th className="p-3 text-center">상태</th><th className="p-3 text-center">코칭</th></tr>
                        </thead>
                            ...
                            return (
                              <tr key={sub.id} className="hover:bg-slate-50">
                                ...
                                <td className="p-3 text-center">
                                  <button onClick={()=>{ setViewingSubmission(sub); setSelectedQuestion(relatedQ); setActiveFeedbackSubmissionId(sub.id); setFeedbackInputText(sub.feedbackText||''); setFeedbackInputImagePreview(sub.feedbackImageUrl||''); }} className="px-2 py-1 bg-indigo-50 hover:bg-indigo-600 hover:text-white rounded text-[10px] font-bold transition-colors">상세 첨삭</button>
                                </td>
                              </tr>
                            )
                          })}
              ...
              <div className="flex flex-col gap-6 animate-fade-in">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                  ...
                  <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold p-3.5 rounded-xl mb-4 flex items-start gap-2">
                    <span className="text-lg leading-none">💡</span> <span>엑셀처럼 여러 명의 학생 정보를 표에 바로 입력하고 [일괄 등록 완료] 버튼을 눌러 한 번에 추가하세요. (초기 비밀번호: 1234)</span>
                  </div>
                      ...
                      <tbody className="divide-y divide-slate-200 bg-white">
                        {draftStudents.map((row, index) => (
                          <tr key={row.rowId} className="focus-within:bg-emerald-50/50 transition-colors">
                            ...
                            <td className="p-0 border-r border-slate-200"><input type="text" value={row.no} onChange={(e) => handleDraftChange(row.rowId, 'no', e.target.value)} className="w-full p-3 outline-none bg-transparent focus:ring-2 focus:ring-emerald-500" placeholder="학번 입력" /></td>
                            <td className="p-0 border-r border-slate-200"><input type="text" value={row.name} onChange={(e) => handleDraftChange(row.rowId, 'name', e.target.value)} className="w-full p-3 outline-none bg-transparent focus:ring-2 focus:ring-emerald-500" placeholder="실명 입력" /></td>
                            <td className="p-0 border-r border-slate-200"><input type="text" value={row.username} onChange={(e) => handleDraftChange(row.rowId, 'username', e.target.value)} className="w-full p-3 outline-none bg-transparent focus:ring-2 focus:ring-emerald-500" placeholder="아이디 입력" /></td>
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="relative w-full flex-1">
                ...
                <input type="text" value={studentQuestionSearch} onChange={e => setStudentQuestionSearch(e.target.value)} placeholder="과목, 단원, 선생님 등 해시태그 검색..." className="w-full pl-9 pr-8 py-2 text-sm rounded-xl border bg-slate-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none" />
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl animate-in zoom-in-95">
            ...
            {authModal.mode === 'student_register' ? (
              <form onSubmit={handleStudentSignUp} className="space-y-3">
                <div><label className="text-xs font-bold text-slate-500">학번 (예: 30101)</label><input type="text" value={signUpNo} onChange={e=>setSignUpNo(e.target.value)} className="w-full p-2.5 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
                <div><label className="text-xs font-bold text-slate-500">이름</label><input type="text" value={signUpName} onChange={e=>setSignUpName(e.target.value)} className="w-full p-2.5 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
                <div><label className="text-xs font-bold text-slate-500">사용할 아이디 (ID)</label><input type="text" value={signUpId} onChange={e=>setSignUpId(e.target.value)} className="w-full p-2.5 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
                <div><label className="text-xs font-bold text-slate-500">비밀번호</label><input type="password" value={signUpPw} onChange={e=>setSignUpPw(e.target.value)} className="w-full p-2.5 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                {authModal.mode === 'teacher_login' && <div className="text-xs font-bold bg-amber-50 text-amber-800 p-2 rounded-lg">💡 데모 계정: ID [teacher], PW [1234]</div>}
                <div><label className="text-xs font-bold text-slate-500">아이디 (ID)</label><input type="text" value={loginIdInput} onChange={e=>setLoginIdInput(e.target.value)} className="w-full p-3 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
                <div><label className="text-xs font-bold text-slate-500">비밀번호</label><input type="password" value={loginPwInput} onChange={e=>setLoginPwInput(e.target.value)} className="w-full p-3 rounded-xl border bg-slate-50 focus:bg-white" required/></div>
                ...
                {currentUser?.role === 'teacher' ? (
                  viewingSubmission ? (
                    <div className="flex flex-col h-full">
                      <h4 className="font-extrabold text-emerald-600 mb-4 flex items-center gap-1 text-sm"><CheckCircle size={16}/> {viewingSubmission.studentName} 학생 풀이 첨삭</h4>
                      <img src={viewingSubmission.imageUrl} alt="학생 풀이" className="h-40 object-contain rounded-xl border mb-4 bg-slate-50 cursor-zoom-in" onClick={()=>openLightbox(viewingSubmission.imageUrl, '학생 풀이')}/>
                      
                      <div className="flex-1 space-y-4">
                        <div className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer ${feedbackInputImagePreview ? 'border-emerald-400 bg-emerald-50' : 'border-slate-300'}`}>
                          {feedbackInputImagePreview ? (
                            <div className="relative">
                              <img src={feedbackInputImagePreview} alt="첨삭" className="max-h-24 mx-auto rounded"/>
                              <button onClick={()=>{setFeedbackInputImagePreview(''); setFeedbackInputImage(null);}} className="absolute top-0 right-0 bg-slate-800 text-white p-1 rounded-full"><X size={12}/></button>
                            </div>
                          ) : (
                        ...
                        <textarea value={feedbackInputText} onChange={e=>setFeedbackInputText(e.target.value)} placeholder="격려와 코멘트를 남겨주세요." className="w-full p-3 rounded-xl border bg-slate-50 text-sm h-24 outline-none focus:ring-2 focus:ring-emerald-500 resize-none"/>
                      </div>
                      <button onClick={handleSaveFeedbackSubmit} className="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl shadow-md mt-4">첨삭 저장 및 전송</button>
                    </div>
                  ) : (
                      ...
                      return (
                        <div className="flex flex-col h-full space-y-5">
                          <div className={`p-4 rounded-2xl flex justify-between items-center ${isMy ? 'bg-indigo-50 border border-indigo-100' : 'bg-slate-50 border border-slate-200'}`}>
                            ...
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${hasFeed ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{targetSub.status}</span>
                          </div>

                          <div className="grid grid-cols-2 gap-3 shrink-0">
                            <div className="border rounded-xl bg-slate-50 text-center relative overflow-hidden group cursor-zoom-in" onClick={()=>openLightbox(targetSub.imageUrl, '풀이 이미지')}>
                              <div className="absolute top-0 w-full bg-black/60 text-white text-[9px] font-bold py-1 z-10">학생 제출본</div>
                              <img src={targetSub.imageUrl} className="h-32 w-full object-cover group-hover:scale-105 transition-transform" alt="풀이"/>
                            </div>
                            <div className="border rounded-xl bg-red-50/30 text-center relative overflow-hidden group cursor-zoom-in" onClick={()=>hasFeed && openLightbox(targetSub.feedbackImageUrl, '교사 첨삭')}>
                              <div className="absolute top-0 w-full bg-red-600/80 text-white text-[9px] font-bold py-1 z-10">선생님 첨삭</div>
                              {hasFeed ? <img src={targetSub.feedbackImageUrl} className="h-32 w-full object-cover group-hover:scale-105 transition-transform" alt="첨삭"/> : <div className="h-32 flex items-center justify-center text-[10px] text-slate-400 font-bold">대기 중 ⏳</div>}
                            </div>
                          </div>
                          ...
                          {hasFeed && (
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 shrink-0">
                              ...
                              <p className="text-xs font-semibold leading-relaxed text-slate-700">{targetSub.feedbackText}</p>
                            </div>
                          )}
                          ...
                          {selectedQuestion.isChallenge && !isMy && (
                            <div className="flex-1 flex flex-col border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm mt-2">
                              ...
                              <form onSubmit={(e)=>handlePeerCommentSubmit(e, targetSub.id)} className="p-2 border-t flex gap-2">
                                <input type="text" value={peerCommentInput} onChange={e=>setPeerCommentInput(e.target.value)} className="flex-1 px-3 py-2 text-xs bg-slate-100 rounded-xl outline-none" placeholder="의견을 입력하세요..." />
