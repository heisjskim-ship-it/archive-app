...
const Loader2 = ({ size = 24, className = "" }) => <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={${className} animate-spin}>;
...
// 💡 이름 익명화 (블라인드) 함수 (예: 홍길동 -> 홍동) const anonymizeName = (name) => { if (!name) return '알수없음'; if (name.length <= 2) return name.charAt(0) + '';
return name.charAt(0) + '*'.repeat(name.length - 2) + name.charAt(name.length - 1);
};
const TUTORIAL_STEPS = {
teacher: [
{ title: '환영합니다, 선생님!', desc: '데이터베이스가 연동된 문제 풀이 아카이브입니다!', icon: },
{ title: '1. 다중 문제 세트 등록', desc: '해시태그를 입력하고, 이미지를 드래그하거나 Ctrl+V로 등록하세요.', icon: },
{ title: '2. 실시간 첨삭 피드백', desc: '학생들이 풀이를 제출하면 현황판에 실시간으로 나타납니다. 직접 첨삭 이미지를 올리고 코멘트를 남겨보세요.', icon: },
{ title: '3. 회원 직접 관리 (엑셀형)', desc: '회원 관리 탭에서 여러 명의 학생을 DB에 한 번에 가입시키고 관리할 수 있습니다.', icon: }
],
student: [
{ title: '반가워요!', desc: '데이터베이스가 연동된 문제 풀이 아카이브입니다!', icon: },
{ title: '1. 해시태그 검색 & 풀이 제출', desc: '종이에 푼 나만의 풀이를 스마트폰으로 사진 찍어 간편하게 제출해보세요.', icon: },
{ title: '2. 선생님의 꼼꼼한 첨삭', desc: '제출한 풀이에 대해 선생님이 피드백을 남겨주시면 내 오답노트에서 볼 수 있습니다.', icon: },
{ title: '3. 공개 챌린지! 친구들과 함께', desc: '챌린지 문제에 참여하면 내 풀이를 공유하고 친구들과 서로 피드백을 주고받을 수 있습니다!', icon: }
]
};
...
/ @type {[string | null, React.Dispatch<React.SetStateAction<string | null>>]} / const [activeFeedbackSubmissionId, setActiveFeedbackSubmissionId] = useState(null); const [feedbackInputText, setFeedbackInputText] = useState(''); /* @type {[File | null, React.Dispatch<React.SetStateAction<File | null>>]} */
...
/ @param {string} msg / const alertMessage = (msg) => { setAlert({ show: true, message: msg }); setTimeout(() => setAlert({ show: false, message: '' }), 3500); }; /* @param {string} imageUrl @param {string} title */
const openLightbox = (imageUrl, title) => setLightbox({ show: true, imageUrl, title });
// 💡 실시간 스냅샷으로부터 현재 뷰잉 중인 학생 풀이 오브젝트 동적 파생 (버그 2번 해결)
...
// 💡 정밀 업그레이드된 이미지 Ctrl+C / Ctrl+V 복사붙여넣기 리스너 (버그 1번 해결)
useEffect(() => {
...
if (tutorial.show || isLoading) return;
  ...
  // 1. clipboardData.files 배열 우선 순위 검사 (바탕화면, 파일 탐색기 등에서 Ctrl+C 복사한 파일 대응)
  ...
  // 2. clipboardData.items 배열 검사 (웹 캡처, 윈도우 스크린샷 캡처 도구 복사본 대응)
  ...
  // 이미지를 정상 획득했다면 브라우저 텍스트 입력의 기본 붙여넣기 기능 방지
  e.preventDefault();
  ...
  if (studentQuestionModal) {
    ...
    if (currentUser?.role === 'student') {
      ...
      if (currentViewingSubmission) {
        ...
        // 풀이를 아예 새로 올리는 중이거나 다시 풀어서 제출하기 활성화 상태
...
const removeQuestionPreview = (index) => setNewQuestion(prev => ({ ...prev, images: prev.images.filter((, i) => i !== index), imagePreviews: prev.imagePreviews.filter((, i) => i !== index) }));
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex flex-col h-full space-y-4">
                      ...
                      {/* 💡 3번 요구사항: 완전히 새로워진 대화형 피드백/의견 교류창 (공간 극대화 및 미려한 디자인 기획) */}
...
풀기 및 확인{q.isPinned && 공지}{q.isChallenge && !q.tags.includes('질문있어요') && 챌린지}{q.tags.includes('질문있어요') && 질문}{q.imageUrls.length > 1 && +{q.imageUrls.length - 1}장}{isSolved && 제출완료}
{q.title}{q.tags.slice(0,3).map((tag, idx) => #{tag})}{q.teacherName} {q.isStudentQuestion ? '학생' : '선생님'}
);
}
}
